...
(function(){
  ...
  function startSession(){
    ...
    $('#sessionBox').style.display='block'; $('#sessionIntro').textContent = `${plan.name} — ${plan.intro}`; renderSessionExercises(); vibrate();
  }
  function clearSession(){ CURRENT=null; $('#sessionBox').style.display='none'; $('#exercises').innerHTML=''; vibrate(); }
  function renderSessionExercises(){ const box=$('#exercises'); box.innerHTML=''; if(!CURRENT) return; CURRENT.exercises.forEach(ex=>{ const row=document.createElement('div'); row.className='ex-row'; const name=document.createElement('div'); name.className='ex-name'; name.textContent=ex.name; row.appendChild(name); const setsBox=document.createElement('div'); setsBox.className='ex-sets'; const minus=document.createElement('button'); minus.className='btn secondary'; minus.textContent='−'; minus.onclick=()=>{ ex.sets=Math.max(0,(ex.sets||0)-1); renderSessionExercises(); vibrate(); }; const cnt=document.createElement('span'); cnt.className='counter'; cnt.textContent=`${ex.sets||0} sets`; const plus=document.createElement('button'); plus.className='btn'; plus.textContent='+'; plus.onclick=()=>{ ex.sets=(ex.sets||0)+1; renderSessionExercises(); vibrate(); }; setsBox.appendChild(minus); setsBox.appendChild(cnt); setsBox.appendChild(plus); row.appendChild(setsBox); box.appendChild(row); }); }
  ...
  function saveSession(){ if(!CURRENT) return; CURRENT.notes=$('#sessNotes').value.trim(); DATA.sessions.push(CURRENT); const setsTotal=totalSets(CURRENT.exercises); DATA.workouts.push({ date: CURRENT.date, type: `Strength — Plan ${CURRENT.planId}`, duration: 0, sets: setsTotal, intensity: 0 }); save(DATA); clearSession(); render(); vibrate(); }
  $('#startSession').addEventListener('click', startSession); $('#clearSession').addEventListener('click', clearSession); $('#saveSession').addEventListener('click', saveSession);
  $('#quickSetBtn').addEventListener('click', ()=>{ DATA.workouts.push({ date: todayISO(), type: 'Strength — Quick', duration: 0, sets: 1, intensity: 0 }); save(DATA); render(); vibrate(); });
  function renderSessions(){ const tb=document.querySelector('#sessionTable tbody'); tb.innerHTML=''; DATA.sessions.slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach((s,idx)=>{ const planName=PLANS[s.planId]?.name || s.planId; const vol=totalVolume(s.exercises); const tr=document.createElement('tr'); tr.innerHTML=`<td>${s.date}</td><td>${planName}</td><td>${totalSets(s.exercises)}</td><td>${Math.round(vol)} kg</td><td></td><td><button class="btn secondary">✖</button></td>`; tr.querySelector('button').onclick=()=>{ DATA.sessions.splice(idx,1); save(DATA); render(); }; tb.appendChild(tr); }); }
  ...
  $('#addChore').addEventListener('click',()=>{ const c={ date: $('#cDate').value || todayISO(), chore: $('#cName').value.trim(), pomodoros: Number($('#cPomo').value||0), minutes: Number($('#cMin').value||0) }; DATA.chores.push(c); save(DATA); $('#cName').value=''; $('#cPomo').value=''; $('#cMin').value=''; render(); vibrate(); });
  ...
  // Render boot
  function render(){ settlePreviousWeekIfNeeded(); const now=new Date(); const {need,counters,allOk,s,e}=weeklyStatus(now); const wExpTotal=DATA.workouts.reduce((a,x)=>a+wExp(x),0); const cExpTotal=DATA.chores.reduce((a,x)=>a+cExp(x),0); const weekWExp=DATA.workouts.filter(x=>inWeek(x.date,s,e)).reduce((a,x)=>a+wExp(x),0); const weekCExp=DATA.chores.filter(x=>inWeek(x.date,s,e)).reduce((a,x)=>a+cExp(x),0); $('#levelsCleared').textContent=DATA.levelsCleared||0; const gs=$('#gateStatus'); gs.textContent=allOk?'Gate Cleared ✅':'Gate Closed ⛔'; $('#totalExp').textContent=Math.round(wExpTotal+cExpTotal); $('#weekExp').textContent=Math.round(weekWExp+weekCExp); $('#weekRange').textContent=`${iso(s)} → ${iso(e)}`; const ul=$('#mqList'); ul.innerHTML=''; addQuestItem(ul,'Strength Workouts',`${counters.strength}/2`,need.strength2); addQuestItem(ul,'Extra Workout (any/cardio)',`${counters.extra}/1`,need.extraWorkout); addQuestItem(ul,'House Clean',`${counters.clean}/4`,need.fullClean); addQuestItem(ul,'Walk',`${counters.walk}/1`,need.oneWalk); addQuestItem(ul,'Reading',`${counters.readH}/2`,need.read2h); renderWorkouts(); renderChores(); renderSessions(); renderCalendar(); updateWaterBar(); $('#cfgBaseWorkout').value=DATA.config.baseWorkout||50; $('#cfgBaseChore').value=DATA.config.baseChore||20; $('#cfgPerMin').value=DATA.config.perMin||2; $('#cfgWaterTarget').value=DATA.config.waterTarget||2.0; const kw=DATA.config.kw||{}; $('#kwStrength').value=kw.strength||''; $('#kwCardio').value=kw.cardio||''; $('#kwWalk').value=kw.walk||''; $('#kwVac').value=kw.vacuum||''; $('#kwSweep').value=kw.sweep||''; $('#kwDust').value=kw.dust||''; $('#kwBath').value=kw.bath||''; $('#kwRead').value=kw.read||''; $('#sessDate').value=todayISO(); $('#qDate').value=todayISO(); }
  function addQuestItem(ul,title,counter,ok){ const li=document.createElement('li'); const dot=document.createElement('span'); dot.className='dot'; li.appendChild(dot); li.appendChild(document.createTextNode(` ${title} — ${counter}`)); if(!ok){ li.style.opacity=.85; li.style.borderColor='#3a2630'; } ul.appendChild(li); }
  ...
  $('#exportBtn').addEventListener('click',()=>{ const blob=new Blob([JSON.stringify(DATA,null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='solo_leveling_weekly_pwa_v3_2_1_save.json'; a.click(); });
