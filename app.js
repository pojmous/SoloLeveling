...
(function(){
  ...
  const storeKey='solo_leveling_weekly_gate_pwa_v3';

  const defaultData = () => ({
    config:{ baseWorkout:50, baseChore:20, perMin:2,
  ...
  const PLANS = {
    A: {
      id:'A', name:'Back/Shoulder & Abs', intro:'Start: 5 min stairs. Choose ~3 back exercises, one lower-back, one shoulder, plus abs.',
      exercises:[
        {name:'Stairs (warm-up 5 min)', fixed:true},
        {name:'Seated Cable Row', choosable:true},
        {name:'Pull-ups', choosable:true},
        {name:'Lat Pulldown', choosable:true},
        {name:'Lower Back (e.g., Back Extensions)', fixed:false},
        {name:'Shoulder (choice)', fixed:false},
        {name:'Abs', fixed:false}
      ]
    },
    B: {
      id:'B', name:'Chest/Calves & Shoulder + Abs', intro:'Start: 5 min stairs, warm-up. Then HS Bench, Pec Deck, Pullover, Shoulder (choice), Standing Calves, Abs.',
      exercises:[
        {name:'Stairs (warm-up 5 min)', fixed:true},
        {name:'General Warm-up', fixed:true},
        {name:'Hammer Strength Bench Press', fixed:true},
        {name:'Pec Deck', fixed:true},
        {name:'Pullover', fixed:true},
        {name:'Shoulder (choice)', fixed:false},
        {name:'Standing Calves', fixed:true},
        {name:'Abs', fixed:false}
      ]
    }
  };

  let CURRENT = null; // {date, planId, exercises:[{name, sets}], notes}

  function startSession(){
    const pid = document.getElementById('planSelect').value; const plan = PLANS[pid];
    CURRENT = { date: document.getElementById('sessDate').value || todayISO(), planId: pid, notes:'', exercises: plan.exercises.map(e=>({name:e.name, sets:0})) };
  ...
  function renderSessionExercises(){
    ...
    CURRENT.exercises.forEach((ex, i)=>{
      ...
      const minus = document.createElement('button'); minus.className='btn secondary'; minus.textContent='−'; minus.onclick=()=>{ ex.sets=Math.max(0,Number(ex.sets||0)-1); renderSessionExercises(); };
      const cnt = document.createElement('span'); cnt.className='counter'; cnt.textContent = `${ex.sets||0} sets`;
      const plus = document.createElement('button'); plus.className='btn'; plus.textContent='+'; plus.onclick=()=>{ ex.sets=Number(ex.sets||0)+1; renderSessionExercises(); };
      setsBox.appendChild(minus); setsBox.appendChild(cnt); setsBox.appendChild(plus);
      row.appendChild(setsBox);
      box.appendChild(row);
    });
  }
  ...
  function saveSession(){
    if(!CURRENT) return;
    CURRENT.notes = document.getElementById('sessNotes').value.trim();
    DATA.sessions.push(CURRENT);
    // Also log a generic Strength workout for weekly counters
  ...
  function renderSessions(){
    ...
    DATA.sessions.slice().sort((a,b)=>a.date.localeCompare(b.date)).forEach((s,idx)=>{
      const planName = PLANS[s.planId]?.name || s.planId;
      const exShort = s.exercises.map(e=>`${e.name} (${e.sets})`).join(', ');
      const tr=document.createElement('tr');
      tr.innerHTML = `<td>${s.date}</td><td>${planName}</td><td>${totalSets(s.exercises)}</td><td>${exShort}</td><td><button class="btn secondary">✖</button></td>`;
      tr.querySelector('button').addEventListener('click',()=>{ DATA.sessions.splice(idx,1); save(DATA); render(); });
      tb.appendChild(tr);
    });
  }
  ...
  function isCleanBoy(n){ return !!(n && n.clean && Number(n.water)>=2.0); }

  function monthStats(year, month){ // month: 0-11
    const daysInMonth = new Date(year, month+1, 0).getDate();
    let count=0, streak=0, best=0; let prevClean=false;
    for(let d=1; d<=daysInMonth; d++){
      const ds = `${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
      const good = isCleanBoy(getNutrition(ds));
      if(good){ streak = prevClean? streak+1 : 1; count++; prevClean=true; } else { best = Math.max(best, streak); streak=0; prevClean=false; }
    }
    best = Math.max(best, streak);
    return {daysInMonth, count, best, streak};
  }
  ...
  function renderCalendar(){
    ...
    // Monday=0..Sunday=6
    const jsDow = first.getDay(); // 0 Sunday .. 6 Saturday
    const offset = (jsDow+6)%7; // convert to Monday-first index
    ...
    ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].forEach(d=>{ const h=document.createElement('div'); h.className='head'; h.textContent=d; head.appendChild(h); });
    ...
    for(let d=1; d<=days; d++){
      ...
      const rec = getNutrition(ds);
      const good = isCleanBoy(rec);
      const mark = document.createElement('span'); mark.className='mark ' + (rec? (good?'good':'bad') : 'none'); mark.textContent = rec? (good?'CB':'X') : '—';
      const dn = document.createElement('div'); dn.className='d'; dn.textContent = d;
      cell.appendChild(dn); cell.appendChild(mark);
      cell.addEventListener('click', ()=>{
        document.getElementById('qDate').value = ds; document.getElementById('qClean').checked = !!(rec && rec.clean); document.getElementById('qWater').value = rec? rec.water: '';
      });
    ...
    document.getElementById('qMonthCount').textContent = st.count;
    document.getElementById('qStreak').textContent = st.streak;
  }
  ...
  function render(){
    ...
    const gs = document.getElementById('gateStatus'); gs.textContent = allOk ? 'Gate Cleared ✅' : 'Gate Closed ⛔'; gs.className = 'value ' + (allOk?'ok':'fail');
  ...
  document.getElementById('cfgBaseWorkout').value = DATA.config.baseWorkout||50;
  document.getElementById('cfgBaseChore').value = DATA.config.baseChore||20;
  document.getElementById('cfgPerMin').value = DATA.config.perMin||2;
  const kw = DATA.config.kw||{};
  document.getElementById('kwStrength').value = kw.strength||'';
  document.getElementById('kwCardio').value = kw.cardio||'';
  document.getElementById('kwWalk').value = kw.walk||'';
  document.getElementById('kwVac').value = kw.vacuum||'';
  document.getElementById('kwSweep').value = kw.sweep||'';
  document.getElementById('kwDust').value = kw.dust||'';
  document.getElementById('kwBath').value = kw.bath||'';
  document.getElementById('kwRead').value = kw.read||'';
