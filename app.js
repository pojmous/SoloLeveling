...
(function(){
  ...
  const defaultData = () => ({
    ...
    workouts:[], chores:[], nutrition:[], levelsCleared:0, lastWeekISO: iso(startOfWeek()),
  });
  ...
  function classifyWorkout(minutes, sets, intensity){
    if(Number(sets)>0) return 'Strength';
    if(Number(minutes)>=10 && Number(intensity)>=2) return 'Cardio';
    if(Number(minutes)>0) return 'Walk';
    return 'Workout';
  }
  ...
  $('#mWSave').addEventListener('click', ()=>{
    const minutes = Number($('#mWMin').value||0);
    const sets = Number($('#mWSets').value||0);
    const intensity = Number($('#mWInt').value||3);
    const type = classifyWorkout(minutes, sets, intensity);
    DATA.workouts.push({ date: todayISO(), type, duration: minutes, sets, intensity });
    save(DATA); closeModal('#workoutModal'); renderWeekly();
  });

  $('#mCSave').addEventListener('click', ()=>{
    const minutes = Number($('#mCMin').value||0);
    const day = todayISO();
    const add = (name) => DATA.chores.push({ date: day, chore: name, minutes });
    if($('#cVac').checked) add('Vacuum');
    if($('#cSweep').checked) add('Sweep');
    if($('#cDust').checked) add('Dust');
    if($('#cBath').checked) add('Bathroom');
    if($('#cRead').checked) add('Reading');
    save(DATA); closeModal('#choreModal'); renderWeekly();
  });

  // Clean Boy events
  $$('button[data-ml]').forEach(b=> b.addEventListener('click', ()=>{ const addL=Number(b.getAttribute('data-ml'))/1000; const d=$('#qDate').value||todayISO(); const rec=getNut(d)||{date:d,clean:false,water:0}; const nw=Number(rec.water||0)+addL; upsertNutrition(d, rec.clean, nw); $('#qWater').value=nw.toFixed(2); updateWaterBar(); }));
  $('#waterCustomBtn').addEventListener('click', ()=>{ const v=prompt('Add water (ml):','200'); if(!v) return; const n=Number(v); if(!isNaN(n) && n>0){ const d=$('#qDate').value||todayISO(); const rec=getNut(d)||{date:d,clean:false,water:0}; const nw=Number(rec.water||0)+n/1000; upsertNutrition(d, rec.clean, nw); $('#qWater').value=nw.toFixed(2); updateWaterBar(); }});
  ...
  function init(){
    settleWeek();
    renderWeekly();
    let y=new Date().getFullYear(), m=new Date().getMonth(); cal={year:y, month:m};
    $('#qDate').value = todayISO(); renderCalendar(); updateWaterBar();
    // Prefill settings modal fields
