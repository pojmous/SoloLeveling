...
(function(){
  // Ensure code only runs after DOM is ready
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCalendar(){
      const y=cal.year,m=cal.month; const first=new Date(y,m,1); const days=new Date(y,m+1,0).getDate(); const jsDow=first.getDay(); const offset=(jsDow+6)%7;
      ...
      const grid=$('#calGrid'); grid.innerHTML=''; for(let i=0;i<offset;i++){ const pad=document.createElement('div'); pad.style.minHeight='78px'; grid.appendChild(pad); }
      for(let d=1; d<=days; d++){
        ...
        // CB mark
        const rec = getNut(ds); const good=isCB(rec); const mark=document.createElement('span'); mark.className='mark ' + (rec? (good?'good':'bad') : 'none'); mark.textContent = rec? (good?'CB':'X') : '—'; cell.appendChild(mark);
        // Emoji icons for activities
        const icons=document.createElement('div'); icons.className='icons'; icons.textContent=getIconsForDay(ds); cell.appendChild(icons);
        cell.onclick=()=>{ $('#qDate').value = ds; $('#qClean').checked = !!(rec && rec.clean); $('#qWater').value = rec? rec.water : ''; updateWaterBar(); };
        grid.appendChild(cell);
      }
      const st = (function(){ let count=0,streak=0,best=0; for(let d=1; d<=days; d++){ const ds=`${y}-${String(m+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`; const good=isCB(getNut(ds)); if(good){ streak++; count++; best=Math.max(best,streak);} else { streak=0; } } return {count,streak,best}; })();
      $('#qMonthCount').textContent=st.count; $('#qStreak').textContent=st.streak; $('#qBest').textContent=st.best;
    }

    function getIconsForDay(ds){
      const w = DATA.workouts.filter(x=>x.date===ds);
      const c = DATA.chores.filter(x=>x.date===ds);
      let s='';
      if (w.some(x=>/Strength/i.test(x.type||''))) s += '🏋️';
      if (w.some(x=>/Cardio/i.test(x.type||''))) s += '🏃';
      if (w.some(x=>/Walk/i.test(x.type||''))) s += '🚶';
      if (c.some(x=>/Reading/i.test(x.chore||''))) s += '📚';
      if (c.some(x=>!(/Reading/i.test(x.chore||'')))) s += '🧹';
      const rec = getNut(ds); if (rec && rec.water>0) s += '💧'; if (isCB(rec)) s += '🥗';
      return s;
    }
    ...
    const ctaW = $('#ctaWorkout'); const ctaC = $('#ctaChore');
    if (ctaW) ctaW.addEventListener('click', ()=> openModal('#workoutModal'));
    if (ctaC) ctaC.addEventListener('click', ()=> openModal('#choreModal'));

    const mWClose=$('#mWClose'); if(mWClose) mWClose.addEventListener('click', ()=> closeModal('#workoutModal'));
    const mCClose=$('#mCClose'); if(mCClose) mCClose.addEventListener('click', ()=> closeModal('#choreModal'));

    // Quick add workout
    const mWSave=$('#mWSave'); if(mWSave) mWSave.addEventListener('click', ()=>{ const minutes=Number($('#mWMin').value||0), sets=Number($('#mWSets').value||0), intensity=Number($('#mWInt').value||3); const type=classifyWorkout(minutes, sets, intensity); DATA.workouts.push({ date: todayISO(), type, duration: minutes, sets, intensity }); save(DATA); closeModal('#workoutModal'); renderWeekly(); renderCalendar(); });
    ...
    const addSetBtn=$('#addSetBtn'); if(addSetBtn) addSetBtn.addEventListener('click', ()=>{ if(!LIVE) return; const ex=$('#liveEx').value; const kg=Number($('#liveKg').value||0); const reps=Number($('#liveReps').value||0); if(!(kg>0 && reps>0)) return; LIVE.sets.push({ex, kg, reps}); const chip=document.createElement('span'); chip.className='set'; chip.textContent = `${ex}: ${kg}×${reps}`; $('#setsBox').appendChild(chip); updateLiveStats(); });

    const finishLiveBtn=$('#finishLiveBtn'); if(finishLiveBtn) finishLiveBtn.addEventListener('click', ()=>{ if(!LIVE) return; const totalSets = LIVE.sets.length; const volume = LIVE.sets.reduce((a,s)=>a+s.kg*s.reps,0); const type = `Strength — ${LIVE.group}`; DATA.workouts.push({ date: LIVE.date, type, duration: 0, sets: totalSets, intensity: 0, details: { volume, sets: LIVE.sets } }); save(DATA); LIVE=null; closeModal('#workoutModal'); renderWeekly(); renderCalendar(); });

    // Chores add
    const mCSave=$('#mCSave'); if(mCSave) mCSave.addEventListener('click', ()=>{ const minutes=Number($('#mCMin').value||0); const day=todayISO(); const add=n=>DATA.chores.push({ date: day, chore: n, minutes }); if($('#cVac').checked) add('Vacuum'); if($('#cSweep').checked) add('Sweep'); if($('#cDust').checked) add('Dust'); if($('#cBath').checked) add('Bathroom'); if($('#cRead').checked) add('Reading'); save(DATA); closeModal('#choreModal'); renderWeekly(); renderCalendar(); });

    // Settings
    const settingsBtn=$('#settingsBtn'); if(settingsBtn) settingsBtn.addEventListener('click', ()=> openModal('#settingsModal'));
    const closeSettings=$('#closeSettings'); if(closeSettings) closeSettings.addEventListener('click', ()=> closeModal('#settingsModal'));
    const saveCfg=$('#saveCfg'); if(saveCfg) saveCfg.addEventListener('click',()=>{ DATA.config.baseWorkout=Number($('#cfgBaseWorkout').value||50); DATA.config.baseChore=Number($('#cfgBaseChore').value||20); DATA.config.perMin=Number($('#cfgPerMin').value||2); DATA.config.waterTarget=Number($('#cfgWaterTarget').value||2.0); save(DATA); closeModal('#settingsModal'); renderWeekly(); });

    // Clean Boy controls
    function addWaterML(ml){ const addL=Number(ml)/1000; const d=$('#qDate').value||todayISO(); const rec=getNut(d)||{date:d,clean:false,water:0}; const nw=Number(rec.water||0)+addL; upsertNutrition(d, rec.clean, nw); $('#qWater').value=nw.toFixed(2); updateWaterBar(); renderCalendar(); }
    $$('button[data-ml]').forEach(b=> b.addEventListener('click', ()=> addWaterML(b.getAttribute('data-ml'))));
    const waterCustomBtn=$('#waterCustomBtn'); if(waterCustomBtn) waterCustomBtn.addEventListener('click', ()=>{ const v=prompt('Add water (ml):','200'); if(!v) return; const n=Number(v); if(!isNaN(n) && n>0) addWaterML(n); });
    const qSave=$('#qSave'); if(qSave) qSave.addEventListener('click', ()=>{ const d=$('#qDate').value||todayISO(); const c=$('#qClean').checked; const w=Number($('#qWater').value||0); upsertNutrition(d,c,w); renderWeekly(); renderCalendar(); });

    // Month nav
    const prevMonth=$('#prevMonth'); if(prevMonth) prevMonth.addEventListener('click',()=>{ cal.month--; if(cal.month<0){cal.month=11; cal.year--; } renderCalendar(); });
    const nextMonth=$('#nextMonth'); if(nextMonth) nextMonth.addEventListener('click',()=>{ cal.month++; if(cal.month>11){cal.month=0; cal.year++; } renderCalendar(); });
    ...
    function init(){
      ...
      renderWeekly(); renderCalendar(); updateWaterBar();
      // hide the boot warning now that JS definitely ran
      const warn=document.getElementById('bootWarn'); if(warn) warn.style.display='none';
    }
