...
(function(){
  ...
  const defaultData = () => ({ config:{ baseWorkout:50, baseChore:20, perMin:2, waterTarget:2.0 }, workouts:[], chores:[], nutrition:[], levelsCleared:0, lastWeekISO: iso(startOfWeek()) });
  ...
  function init(){ settleWeek(); renderWeekly(); let y=new Date().getFullYear(), m=new Date().getMonth(); cal={year:y, month:m}; $('#qDate').value = todayISO(); renderCalendar(); updateWaterBar(); $('#cfgBaseWorkout').value=DATA.config.baseWorkout||50; $('#cfgBaseChore').value=DATA.config.baseChore||20; $('#cfgPerMin').value=DATA.config.perMin||2; $('#cfgWaterTarget').value=DATA.config.waterTarget||2.0; $('#prevMonth').addEventListener('click',()=>{ cal.month--; if(cal.month<0){cal.month=11; cal.year--; } renderCalendar(); }); $('#nextMonth').addEventListener('click',()=>{ cal.month++; if(cal.month>11){cal.month=0; cal.year++; } renderCalendar(); }); }
